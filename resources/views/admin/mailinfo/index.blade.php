...
@section('main')
    ...
    <section class="content-header">
        <h1>
            Mail Infor
            <small>Detail</small>
            <a href="{{ route('mailinfo.create-category') }}" class="btn btn-info text-uppercase">Create Category</a>
                        ...
                        <div class="row">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                                        ...
                                        @foreach ($allMessages as $message)
                                            <tr class="{{ $j%2 == 0 ? 'even' : 'odd' }} no-wrap-container" role="row">
                                                <td>{{ $j++ }}</td>
                                                <td>{{ $message->created_at }}</td>
                                                <td>
                                                    @php
                                                        $toEmails = json_decode($message->toEmail);
                                                        $toEmailStr = $toEmails[0];

                                                        for($i=1;$i<count($toEmails);$i++) {
                                                            $toEmailStr .= ", " . $toEmails[$i];
                                                        }
                                                        if(strlen($toEmailStr) > 30)
                                                        {
                                                            echo substr($toEmailStr,30);
                                                        }
                                                        else {
                                                            echo $toEmailStr;
                                                        }
                                                ...
                                                <td>
                                                    @if(!$message->fileUrl)
                                                        Empty
                                                    @else
                                                        <a href="{{ asset($message->fileUrl) }}">Read Me</a>
                                                    @endif
                                                </td>
                                                <td><a href="{{ url('admin/mailinfo/show-message?message_id=' . $message->id) }}" target="_blank">More...</a></td>
                                            </tr>
                                        @endforeach
                                        </tbody>
                                        <tfoot>
                            ...
                            <div class="col-sm-5">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        <table id="searchable_table2" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                                            <thead>
                                                <tr class="no-wrap-container">
                                                    <th>S.No</th>
                                                    <th>Title</th>
                                            ...
                                            <tbody>
                                                ...
                                                @foreach ($mailerCategories as $category)
                                                    <tr class="{{ $i%2 == 0 ? 'even' : 'odd' }} no-wrap-container" role="row">
                                                        ...
                                                        <td>
                                                            <a href="{{ url('admin/mailinfo/get-category?category_id='.$category->id) }}" style="margin-top: .3rem">
                                                                More...
                                                            </a>
                                                        </td>
                                                    </tr>
